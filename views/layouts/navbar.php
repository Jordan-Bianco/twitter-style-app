...
<nav class="md:px-8 px-4 py-4 flex items-center justify-between space-x-6 mb-3">
    ...
    <div class="md:w-2/4 w-full">
        <form action="/users" method="get" class="relative">
            ...
            <svg class="w-5 h-5 text-sky-500 absolute top-2.5 left-3.5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="md:w-1/4 max-w-max flex items-center justify-end space-x-2">
        <?php if (!Session::isLoggedIn()) : ?>
            <div class="flex items-center space-x-2">
                <a href="/login" class="block hover:text-sky-500 <?= Application::$app->request->getUri() === '/login' ? 'text-sky-500' : '' ?>">Log in</a>
                <a href="/register" class="block hover:text-sky-500 <?= Application::$app->request->getUri() == '/register' ? 'text-sky-500' : '' ?>">Sign in</a>
            </div>
        <?php else : ?>
            <a class="hover:text-sky-500" href="/<?= Application::$app->session->get('user')['username'] ?>">
