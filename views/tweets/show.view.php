...
<div class="max-w-lg mx-auto">
    ...
    <!-- Tweet -->
    <div class="p-4 panel space-y-2">
        <header class="flex justify-between items-center mb-2.5">
            <a href="/<?= $tweet['username'] ?>" class="flex items-center space-x-2">
                <img src="https://eu.ui-avatars.com/api/?name=<?= $tweet['username'] ?>" alt="user_avatar" class="w-8 h-8 rounded-lg flex-none">
        ...
        <div class="mt-8">
            <?php foreach ($comments as $comment) : ?>
                <div class="panel mb-3 space-y-2.5">
                    <header class="flex justify-between items-center mb-2.5">
                        <a href="/<?= $comment['username'] ?>" class="flex items-center space-x-2">
                            <img src="https://eu.ui-avatars.com/api/?name=<?= $comment['username'] ?>" alt="user_avatar" class="w-7 h-7 rounded-lg flex-none">
                        ...
                        <?php if ($comment['user_id'] === Application::$app->session->get('user')['id']) : ?>
                            <div class="flex items-center space-x-1">
                                ...
                                <a href="/comments/<?= $comment['id'] ?>/edit">
                                    <svg class="w-[18px] h-[18px] text-zinc-400 hover:text-zinc-500 cursor-pointer" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z">
                                        </path>
                                    </svg>
                                </a>
                                <!-- Delete -->
                                <form action="/comments/<?= $comment['id'] ?>/delete" method="post" class="flex">
